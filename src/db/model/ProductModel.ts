  ...
  static async generateEmbedding(text: string): Promise<number[]> {
    try {
      const response = await fetch(
        `https://generativelanguage.googleapis.com/v1beta/models/${process.env
          .GEMINI_EMBEDDING!}:embedContent?key=${process.env.GOOGLE_API_KEY!}`,
        {
          method: "POST",
          headers: {
  ...
  static async updateProduct(id: string, payload: Partial<ProductType>) {
    ...
    if (parsed.name || parsed.description) {
      ...
      const textForEmbedding = `${parsed.name || product.name} ${
        parsed.description || product.description
      }`;

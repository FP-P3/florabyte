...
const userSchema = z.object({
  name: z.string({ message: "Name is required" })  // Ganti required_error dengan message
          .trim()
          .min(1, { message: "Name is required" }),
  username: z.string({ message: "Username is required" })  // Ganti required_error dengan message
            .trim()
            .min(3, { message: "Username must be at least 3 characters" }),
  password: z.string({ message: "Password is required" })  // Ganti required_error dengan message
...
class UserModel {
  static async createUser(payload: Partial<UserType>) {
    ...
    const existing = await db.collection("users").findOne({ username: parsed.username })
    if (existing) {

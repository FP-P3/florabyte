...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {  // Update tipe params ke Promise
  try {
    ...
    const product = await db.collection("Products").findOne({ _id: new (require('mongodb').ObjectId)(id) });
    if (!product) throw { message: "Product not found", status: 404 };

    let cloudinaryDeletion: any = null;
    if (product.imgUrl && typeof product.imgUrl === 'string') {
      try {
        ...
        if (uploadIndex !== -1) {
          ...
          publicIdWithExt = urlPath.startsWith('/') ? urlPath.slice(1) : urlPath;
        }
        ...
        if (publicId) {
          cloudinaryDeletion = await cloudinary.uploader.destroy(publicId).catch(err => ({ error: err?.message || 'cloudinary delete failed' }));
        }
      } catch (e) {

...
function isPromise<T>(obj: unknown): obj is Promise<T> {
  return (
    typeof obj === "object" &&
    obj !== null &&
    "then" in (obj as Record<string, unknown>) &&
...
export async function GET(
  ...
  try {
    ...
    const maybeParams = (
      context as { params: { id: string } | Promise<{ id: string }> }
    ).params;
    const resolvedParams = isPromise<{ id: string }>(maybeParams)
      ? await maybeParams
      : maybeParams;
    ...
    const orders: OrderResponseItem[] = carts.map((c) => {
      ...
      const status: OrderResponseItem["status"] =
        (c.orderStatus as OrderResponseItem["status"]) ||
        (c.status === "paid" ? "Pending" : "Dibatalkan");
      const paymentStatus: OrderResponseItem["paymentStatus"] =
        c.status === "paid" ? "paid" : "cancelled";
      return {
        id: c._id.toString(),
        items,
